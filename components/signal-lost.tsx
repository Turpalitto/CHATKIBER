...
export function SignalLost({ reason, onContinue }: SignalLostProps) {
  ...
  return (
    <div className="signal-panel w-full max-w-2xl rounded-[32px] p-8 text-center sm:p-10">
      <p className="text-[11px] uppercase tracking-[0.34em] text-white/42">{m.lost.eyebrow}</p>
      <h2 className="display-font mt-6 text-4xl text-white sm:text-5xl">{m.lost.title}</h2>
      <p className="mx-auto mt-5 max-w-xl text-base leading-8 text-white/68 sm:text-lg">{m.lost.body}</p>
