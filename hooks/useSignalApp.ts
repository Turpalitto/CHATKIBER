...
import { getRandomFrequency, getTodaysFrequency, simulateOnlineCount } from "@/lib/frequency";
import { moderateMessage } from "@/lib/moderation";
import { createSignalEngine, SignalEngine } from "@/lib/signal-engine";
import { uid, wait } from "@/lib/utils";
import { AppStage, Frequency, FrequencyKind, Message, ModeOption, ModerationResult, ToneOption, VoiceDiagnosticsShareResult, VoiceQosExportResult, VoiceQosRecommendationsResult, VoiceQosReportResult, VoiceQosSample, WebRtcSignalMessage } from "@/lib/types";

function buildIntroMessage(frequency: Frequency): Message {
  return {
    id: uid("msg"),
    sender: "system",
    type: "system",
    text: frequency.kind === "daily"
      ? `Frequency #${frequency.number} • ${frequency.prompt}`
      : `Random Signal • ${frequency.prompt}`,
    createdAt: Date.now()
  };
}

function buildSystemMessage(text: string): Message {
  return {
    id: uid("msg"),
    sender: "system",
    type: "system",
...
export function useSignalApp() {
  ...
  const [dailyFrequency, setDailyFrequency] = useState<Frequency>(() => getTodaysFrequency());
  const [randomFrequency, setRandomFrequency] = useState<Frequency>(() => getRandomFrequency(0.421337));
  ...
  const [partnerLabel, setPartnerLabel] = useState("Unknown Soul");
  ...
  useEffect(() => {
    ...
    void ensureAnonTokenHash();
    setOnlineCount(simulateOnlineCount(Date.now()));
    setRandomFrequency(getRandomFrequency(Math.random()));
    ...
    const interval = setInterval(() => {
      ...
      setOnlineCount(simulateOnlineCount(Date.now()));
      setDailyFrequency(getTodaysFrequency());
    }, 6000);
    ...
    return () => {
      ...
      if (engineRef.current) {
        void engineRef.current.disconnect("Session closed.");
  ...
  const applyViolation = useCallback((result: ModerationResult) => {
    setWarning(result);
    if (result.status === "block") {
      setViolationCount((current) => {
        ...
        if (next >= 2) {
          void (async () => {
            if (engineRef.current) {
              await engineRef.current.disconnect("Connection ended by safety system.");
              engineRef.current = null;
            }
            setTyping(false);
            setLatestWebRtcSignal(null);
            setDisconnectReason("Connection ended by safety system.");
            setStage("lost");
          })();
        }
        return next;
  ...
  const enterChat = useCallback((nextPartnerLabel: string, frequency: Frequency) => {
    const intro = buildIntroMessage(frequency);
  ...
  const chooseFrequency = useCallback((kind: FrequencyKind) => {
    resetWarnings();
    setActiveFrequency(kind === "daily" ? dailyFrequency : randomFrequency);
    setStage("intent");
  }, [dailyFrequency, randomFrequency, resetWarnings]);
  ...
  const findAnotherSignal = useCallback(() => {
    ...
    resetWarnings();
    setStage("frequency");
  }, [clearEngine, resetSessionVisuals, resetWarnings]);
  ...
  const cancelWaiting = useCallback(async () => {
    ...
    if (engineRef.current) {
      await engineRef.current.disconnect("Signal search cancelled.");
      engineRef.current = null;
    }
  ...
  const connect = useCallback(async () => {
    ...
    for (let i = 0; i < CONNECTION_STEPS.length; i += 1) {
      ...
      setMessages([
        {
          ...
          sender: "system",
          type: "system",
          text: "Waiting for a compatible signal...",
          createdAt: Date.now()
        }
      ]);
      setStage("waiting");
    } catch (error) {
      const message = error instanceof Error ? error.message : "No clear signal could be locked.";
      setDisconnectReason(message);
      setStage("lost");
    }
  }, [activeFrequency, ensureAnonTokenHash, enterChat, mode, resetSessionVisuals, resetWarnings, tone]);
  ...
  const sendText = useCallback(async (text: string) => {
    ...
    const localModeration = moderateMessage(text);
  ...
  const sendVoicePulse = useCallback(async (level: number) => {
    ...
    setMessages((current) => [
      ...current,
      {
        ...
        sender: "self",
        type: "voice",
        text: level > 0.55 ? "Voice burst transmitted." : "Soft transmission sent.",
        createdAt: Date.now()
      }
    ]);
  ...
  const moderateVoiceTranscript = useCallback(async (transcript: string) => {
    if (!engineRef.current) {
  ...
  const reportVoiceQos = useCallback(async (sample: VoiceQosSample) => {
    if (!engineRef.current) {
      return {
        status: "ok",
        history: [],
  ...
  return {
    ...
    latestWebRtcSignal,
    connectionIndex,
    connectionStep: CONNECTION_STEPS[connectionIndex],
    disconnectReason,
    begin,
    chooseFrequency,
    backToFrequency,
